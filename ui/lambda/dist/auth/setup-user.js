...
const handler = async (event) => {
    ...
    try {
        const initiateAuthCommand = new client_cognito_identity_provider_1.AdminInitiateAuthCommand({
        ...
        if (authResponse.ChallengeName === client_cognito_identity_provider_1.ChallengeNameType.NEW_PASSWORD_REQUIRED) {
            const challengeCommand = new client_cognito_identity_provider_1.AdminRespondToAuthChallengeCommand({
                UserPoolId: process.env.USER_POOL_ID,
                ClientId: process.env.CLIENT_ID,
