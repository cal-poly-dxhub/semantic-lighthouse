...
const messageAdminCreatedUser = (event) => ({
    ...
    <p>Or <a href="${process.env.FRONTEND_URL}/login?email=${encodeURIComponent(event.request.userAttributes.email)}">click here</a> to login</p>
    <p>You will be required to change your password on first login.</p>
  `,
});
const firstUserEmailVerification = (event) => ({
    emailSubject: "Welcome to Semantic Lighthouse",
    emailMessage: `
    <h2>Verify your email address</h2>
    <p>Thank you for signing up for Semantic Lighthouse!</p>
    <p>Please verify your email address by entering the following code:</p>
    <p><strong>Verification Code:</strong> ${event.request.codeParameter} or <a href="${process.env.FRONTEND_URL}/verify?code=${event.request.codeParameter}&username=${encodeURIComponent(event.userName)}">click here to verify</a></p>
...
const handler = async (event) => {
    ...
    // }
        ...
        console.warn("WARN: Unhandled trigger source:", event.triggerSource);
    }
    console.log("INFO: Custom message event response:", JSON.stringify(event, null, 2));
    return event;
};
